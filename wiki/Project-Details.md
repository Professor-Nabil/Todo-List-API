...
In this project you are required to develop a RESTful API
to allow users to manage their to-do list.
...
![Project image preview](./img/todo-list-api-bsrdd.png)
...
This will validate the given details,
...
The token can either be a JWT or a random string that can be used for authentication.
We leave it up to you to decide the implementation details.
...
This will validate the given email and password,
and respond with a token if the authentication is successful.
...
User must send the token received from the login endpoint
in the header to authenticate the request.
...
Just like the create todo endpoint,
user must send the token received.
...
how to design and implement a RESTful API with user authentication.

You will also learn how to work with databases,
handle errors, and implement security measures.
